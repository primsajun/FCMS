...
export default function RegisterPage({ onRegisterSuccess, onBackToLogin }) {
  ...
  useEffect(() => {
    // Combine PREDEFINED_TEAMS with custom_teams from Supabase
    async function fetchTeams() {
      const predefinedNames = PREDEFINED_TEAMS.map(t => t.name);
      
      const { data, error } = await supabase.from('custom_teams').select('name');
      let customNames = [];
      if (!error && data) {
        customNames = data.map(t => t.name);
      }
      
      const allTeams = [...new Set([...predefinedNames, ...customNames])].sort();
      setAvailableTeams(allTeams);
      if (allTeams.length > 0) {
        setTeamName(allTeams[0]);
      }
    }
    fetchTeams();
  }, []);
  ...
  const handleRegister = async (e) => {
    ...
    // Validation for existing team
    if (teamSelectionType === 'existing' && !availableTeams.includes(teamName)) {
      setError("Please select a valid existing team name.");
      return;
    }
    if (teamSelectionType === 'new' && !teamName.trim()) {
    ...
    try {
      ...
      // 2. Determine approval status (Admin is manually created, Coach needs Admin, Player needs Coach)
      // They all start as 'pending'
      ...
      if (role === 'coach' && teamSelectionType === 'new') {
        ...
        if (teamError) {
          console.warn("Failed to create custom team record, it may already exist.", teamError);
        }
      }
    ...
    <form onSubmit={handleRegister} style={{display: 'flex', flexDirection: 'column', gap: '1.25rem'}}>
      ...
      <div style={{display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem'}}>
        ...
        <div>
          ...
          <input type="tel" required value={mobile} onChange={e => setMobile(e.target.value)} style={inputStyle} />
        </div>
      </div>
      ...
      {role === 'coach' && (
        <div style={{display: 'flex', gap: '1rem', marginBottom: '0.5rem'}}>
          ...
          <label style={{display: 'flex', alignItems: 'center', gap: '0.5rem', color: '#fff'}}>
            <input type="radio" checked={teamSelectionType === 'new'} onChange={() => { setTeamSelectionType('new'); setTeamName(''); }} />
            Create New Team
          </label>
        </div>
      )}
      ...
      <div>
        ...
        {teamSelectionType === 'existing' || role === 'player' ? (
          <select required value={teamName} onChange={e => setTeamName(e.target.value)} style={inputStyle}>
          ...
          <input type="text" required placeholder="Enter new team name" value={teamName} onChange={e => setTeamName(e.target.value)} style={inputStyle} />
        )}
      </div>
